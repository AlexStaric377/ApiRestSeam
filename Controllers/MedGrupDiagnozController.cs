...
namespace AppRestSeam.Controllers
{
    ...
    public class MedGrupDiagnozController : ControllerBase
    {
        ...
        [HttpGet("{Edrpou}/{IcdGrDiagnoz}/{IcdKey}/{KodZaklad}")]
        public async Task<ActionResult<MedicalGrDiagnoz>> Get(string Edrpou, string IcdGrDiagnoz, string IcdKey, string KodZaklad)
        {
            List<MedicalGrDiagnoz> _listdiagnoz = new List<MedicalGrDiagnoz>();
            if (Edrpou.Trim() == "0" && IcdGrDiagnoz.Trim() == "0" && IcdKey.Trim() == "0" && KodZaklad.Trim() == "0") { return NotFound(); }
            ...
            {
                _listdiagnoz = await db.MedicalGrDiagnozs.Where(x => x.IcdGrDiagnoz.Contains(IcdGrDiagnoz) == true).OrderBy(x => x.Edrpou).ToListAsync();
            }
            if (IcdKey.Trim() != "0")
            {
            ...
            if (KodZaklad.Trim() != "0")
            {
                _listdiagnoz = await db.MedicalGrDiagnozs.Where(x => x.KodZaklad == KodZaklad).ToListAsync();
            }
        ...
        public async Task<ActionResult<MedicalGrDiagnoz>> Delete(string id)
        {
            ...
            if (Convert.ToInt32(id) == -1)
            {
                ...
                {
                    _detailing = await db.MedicalGrDiagnozs.FirstOrDefaultAsync(x => x.Id == str.Id);
                    if (_detailing != null)
                    {
            ...
            {
                _detailing = await db.MedicalGrDiagnozs.FirstOrDefaultAsync(x => x.Id == Convert.ToInt32(id));
